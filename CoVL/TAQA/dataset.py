...
class VideoDataset(Dataset):
    ...
    def proc_label(self, data):
        tmp = stats.norm.pdf(np.arange(output_dim['USDL-CoVL']), loc=data['final_score'] * (output_dim['USDL-CoVL']-1) / label_max, scale=self.args.std).astype(
            np.float32)
        data['soft_label'] = tmp / tmp.sum()
        tmp = [stats.norm.pdf(np.arange(output_dim['MUSDL-CoVL']), loc=judge_score * (output_dim['MUSDL-CoVL']-1) / judge_max, scale=self.args.std).astype(np.float32)
